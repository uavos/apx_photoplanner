...
aero_photo::PhotoUavModel UavEdit::getUavModel()
{
    aero_photo::PhotoUavModel model(f_flightSpeed->value().toInt(),
                                    aero_photo::D2R(f_maxRoll->value().toInt()));
    return model;
}
...
int UavEdit::calcUavRoll()
{
    ...
    int maxRoll = aero_photo::R2D(model.CalcUavMaxRollOnManeuverR(f_maneuverR->value().toInt()));
    return maxRoll;
}
...
{
    auto model = getUavModel();
    int uavR = model.CalcUavManeuverROnMaxRoll(aero_photo::D2R(f_maxRoll->value().toInt()));
    return uavR;
}
